...
describe("uiSchema", () => {
      ...
      const CustomWidget = (props) => {
        ...
        return (
          <select className={className}>{
            enumOptions.map((option, i) => <option key={i}>{option.value}</option>)
          }</select>
        );
      };
  ...
  describe("ui:focus", () => {
    ...
    const shouldFocus = (schema, uiSchema, el, selector = "input", formData) => {
      const props = {schema, uiSchema};
      if (typeof formData !== "undefined") {
        props.formData = formData;
      }

      const {node} = createFormComponent(props, el);
      expect(node.querySelector(selector)).eql(window.document.activeElement);
    };

    describe("number", () => {
      it("should focus on integer input", () => {
        shouldFocus({type: "integer"}, {"ui:autofocus": true}, div);
      });

      it("should focus on integer input, updown widget", () => {
        shouldFocus({type: "integer"}, {"ui:widget": "updown", "ui:autofocus": true}, div);
      });

      it("should focus on integer input, range widget", () => {
        shouldFocus({type: "integer"}, {"ui:widget": "range", "ui:autofocus": true}, div);
      });

      it("should focus on integer enum input", () => {
        shouldFocus({type: "integer", enum: [1, 2, 3]}, {"ui:autofocus": true}, div, "select");
      });
    });

    describe("string", () => {
      it("should focus on text input", () => {
        shouldFocus({type: "string"}, {"ui:autofocus": true}, div);
      });

      it("should focus on textarea", () => {
        shouldFocus({type: "string"}, {"ui:widget": "textarea", "ui:autofocus": true}, div, "textarea");
      });

      it("should focus on password input", () => {
        shouldFocus({type: "string"}, {"ui:widget": "password", "ui:autofocus": true}, div);
      });

      it("should focus on color input", () => {
        shouldFocus({type: "string"}, {"ui:widget": "color", "ui:autofocus": true}, div);
      });

      it("should focus on email input", () => {
        shouldFocus({type: "string", format: "email"}, {"ui:autofocus": true}, div);
      });

      it("should focus on uri input", () => {
        shouldFocus({type: "string", format: "uri"}, {"ui:autofocus": true}, div);
      });

      it("should focus on data-url input", () => {
        shouldFocus({type: "string", format: "data-url"}, {"ui:autofocus": true}, div);
      });
    });

    describe("object", () => {
      it("should focus on date input", () => {
        shouldFocus({type: "string", format: "date"}, {"ui:autofocus": true}, div);
      });

      it("should focus on date-time input", () => {
        shouldFocus({type: "string", format: "date-time"}, {"ui:autofocus": true}, div);
      });

      it("should focus on alt-date input", () => {
        shouldFocus({type: "string", format: "date"}, {"ui:widget": "alt-date",  "ui:autofocus": true}, div, "select");
      });

      it("should focus on alt-date-time input", () => {
        shouldFocus({type: "string", format: "date-time"}, {"ui:widget": "alt-datetime",  "ui:autofocus": true}, div, "select");
      });
    });

    describe("array", () => {
      it("should focus on multiple files input", () => {
        shouldFocus({type: "array", items: {type: "string", format: "data-url"}}, {"ui:autofocus": true}, div);
      });

      it("should focus on first item of a list of strings", () => {
        shouldFocus({type: "array", items: {type: "string", default: "foo"}}, {"ui:autofocus": true}, div, "input", ["foo", "bar"]);
      });
    ...
    describe("boolean", () => {
      it("should focus on checkbox input", () => {
        shouldFocus({type: "boolean"}, {"ui:autofocus": true}, div);
      });

      it("should focus on radio input", () => {
        shouldFocus({type: "boolean"}, {"ui:widget": "radio", "ui:autofocus": true}, div);
      });

      it("should focus on select input", () => {
        shouldFocus({type: "boolean"}, {"ui:widget": "select", "ui:autofocus": true}, div, "select");
      });
    });
  });
